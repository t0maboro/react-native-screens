...
@implementation RNSHeaderItemSpacerComponentView {
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const react::RNSHeaderItemSpacerIOSProps>();
    _props = defaultProps;
    [self resetProps];
  }
  return self;
...
{
  const auto &newSpacerProps = *std::static_pointer_cast<const react::RNSHeaderItemSpacerIOSProps>(props);
  const auto &oldSpacerProps = *std::static_pointer_cast<const react::RNSHeaderItemSpacerIOSProps>(_props);
  ...
  if (oldSpacerProps.sizing != newSpacerProps.sizing) {
    _isFlexible = newSpacerProps.sizing == react::RNSHeaderItemSpacerIOSSizing::Flexible;
    needsUpdate = YES;
  }
...
{
  return react::concreteComponentDescriptorProvider<react::RNSHeaderItemSpacerIOSComponentDescriptor>();
}
...
Class<RCTComponentViewProtocol> RNSHeaderItemSpacerComponentViewCls(void)
{
  return RNSHeaderItemSpacerComponentView.class;
}
